...
$address = '';
$manger = 'Cyxdak1ng';
$cnt = mysqli_query($conn, "SELECT COUNT(*) as cnt from student where mangerid = '$MANGERSYSTEM'");
$res = mysqli_query($conn, "SELECT id,name,address from student where mangerid = '$MANGERSYSTEM'");
$mangers = mysqli_query($conn, "SELECT distinct(MangerID) , MangerName from mangers where MangerID = '$MANGERSYSTEM'");
if (isset($_POST['id'])) {
    $id = $_POST['id'];
}
if (isset($_POST['name'])) {
...
$sqls = '';
if (isset($_POST['add'])) {
    if (empty($name) || empty($id) || empty($address)) {
